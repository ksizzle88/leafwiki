...
Set up and start the [$project-name] application locally.
...
Starts FastAPI server with hot-reload on port 8000. SQLite database ([$project-name].db) is created automatically on first run.
...
```bash
# Test API health
curl -s http://localhost:8000/api/[$project-name]
...
- Database file created at `backend/$project-name.db` on first API call
